# ============================================================
#  MULTI-FRAME dumbbell discovery — BONDED-LIFETIME   [1051 BUILD]
#
#  This is population_engine_optimal.py adapted for 20250110_HAADF_STEM_150_kx_1051.emd.
#  1053 is the fixed reference; every 1051 parameter below was MEASURED, not assumed:
#    - detector (sigma 0.25 nm, blob scale = 2.4 nm physical diameter, thr 0.16*p99):
#      passed the stationarity check (deconvolved diameter flat at 2.39 nm across the movie)
#    - MAX_STEP = 8 px: measured p95 of frame-to-frame motion (engine default 3 was too
#      tight — covered only the 33rd percentile; 27% of frames were pinned at the cap)
#    - birth/death/floor: KEPT from 1053 — 78% of 1051's nearest-neighbour pairs fall in
#      the 2.5-6.5 nm birth band, so the geometry transfers
#    - tracker: 0 swap collisions over 24 isolated seeds at MAX_STEP=8
#
...
EMD    = "20250110_HAADF_STEM_150_kx_1051.emd"
EMD_DS = "Data/Image/dd081586d2424a169f2f37aa5cf2a1ba/Data"    # 69-frame HAADF movie
...
GSIG_NM   = 0.25                  # gaussian sigma in NANOMETRES
GSIG      = GSIG_NM / PX_NM       # -> 0.384 px on 1051
...
THR_FRAC = 0.22                   # per-frame threshold = THR_FRAC * that frame's p99 -- it was 0.16 before

# ---- tracker geometry: R/RR/LAMBDA on SCALE; MAX_STEP MEASURED (not scaled) ----
R, RR = max(1,round(3*SCALE)), max(2,round(10*SCALE))
MAX_STEP = 8                      # MEASURED p95 of motion (engine default round(12*SCALE)=3
                                  # covered only the 33rd pct of real displacement)
